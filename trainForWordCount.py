...
# exclude header words, date, charsets, time
...
def addToDict(wordDict, word, fname, labelDict):
	...
	for ew in exclusive_words:
		if (word.lower() in ew.lower()) or (ew.lower() in word.lower()):
			return 
	if word.lower() not in exclusive_words or re.match('\d{4}', word)==False or word.lower() not in stop_words:

		# group similar words
		# for k in wordDict.keys():
		# 	if word.lower() in k.lower() or k.lower() in word.lower():
